...
import { deriveVehicleU } from '../../src/scene/train-layer.js';
import { VEHICLE_STATUS } from '../../src/core/rt-parser.js';

// deriveVehicleU reads "the next stop" as the first entry in stopTimeUpdate whose
...
// Where each station falls along one line, keyed the way buildRouteIndex keys it.
const branch = (entries) => ({ stationU: new Map(Object.entries(entries)) });
...
describe('deriveVehicleU direction of travel', () => {
    // Stops run in increasing u along the line, so "ahead" is a larger u.
    const lines = [branch({
    ...
    it('places an approaching train behind the stop it is heading to', () => {
        const { u } = deriveVehicleU(vehicle(anchored(rawSequence, 'S16N')), lines);
    ...
    it('is placed past its target when the sequence still lists passed stops', () => {
        ...
        const { u } = deriveVehicleU(vehicle(rawSequence), lines);
        ...
        expect(u).toBeGreaterThan(0.30);
    });

    it('agrees with the anchored window when the feed lists no passed stops', () => {
        const clean = rawSequence.slice(2); // S16 onward
        const fromRaw = deriveVehicleU(vehicle(clean), lines);
        const fromWindow = deriveVehicleU(vehicle(anchored(clean, 'S16N')), lines);

        expect(fromWindow.u).toBeCloseTo(fromRaw.u, 10);
    });
});

describe('deriveVehicleU branch selection', () => {
    ...
    it('chooses the branch carrying the stop ahead', () => {
        const { lineIndex } = deriveVehicleU(vehicle(anchored(rawSequence, 'S16N')), lines);
        expect(lineIndex).toBe(1);
    });

    it('chooses the branch behind it when the sequence lists passed stops', () => {
        const { lineIndex } = deriveVehicleU(vehicle(rawSequence), lines);
        expect(lineIndex).toBe(0);
    });
});

describe('deriveVehicleU degraded inputs', () => {
    const lines = [branch({ S16: 0.30, S17: 0.40 })];
